...
| `mkdocs.yml`, `docs/index.md`, `docs/api.md` (create) | MkDocs + Material + mkdocstrings site assembling all docs + auto-generated API reference |
...
### Task 14: MkDocs site — auto-generated documentation website

Explicit user request: a consolidated Python doc tool that produces an actual
website, not just markdown files. `MkDocs` + `mkdocs-material` (theme/search/
nav) + `mkdocstrings[python]` (auto API reference straight from docstrings)
is the standard choice in the Python ecosystem for exactly this (used by
FastAPI, Pydantic, HTTPX, etc.). It assembles everything already produced by
this plan — `README.md`, `AGENTS.md`, `docs/ROADMAP.md`, the design spec,
this plan, and the generated `docs/MCP_TOOLS.md` (Task 0/13) — plus a live
API reference generated from the docstrings in `db.py`/`llm.py`/`rerank.py`/
`vault.py`/`knowledge.py`, into one static, searchable site.

**Files:**
- Modify: `pyproject.toml` (add `mkdocs`, `mkdocs-material`, `mkdocstrings[python]` to `dev` optional deps)
- Modify: `.gitignore` (ignore the built site)
- Create: `mkdocs.yml`
- Create: `docs/index.md`
- Create: `docs/api.md`

**Interfaces:**
- Consumes: `docs/MCP_TOOLS.md` (Task 0/13), `docs/ROADMAP.md`, `README.md`, `AGENTS.md`, and docstrings on every function/class in `src/minimax_mcp/{db,llm,rerank,vault,knowledge}.py` (Tasks 2-12) — no code changes required there as long as functions keep their one-line docstrings.
- Produces: `uv run mkdocs serve` (local live-reloading site, default `http://127.0.0.1:8000`) and `uv run mkdocs build` (static site in `site/`, not committed). No other task depends on this one.

- [ ] **Step 1: Add MkDocs dependencies to `pyproject.toml`**

In the `dev` optional-dependencies group (alongside `pytest`, `ruff`):
...
- [ ] **Step 2: Ignore the built site in `.gitignore`**

Append after the `# OS / editor` section:

```
# Docs site build output (generated by mkdocs build)
site/
```
...
```yaml
site_name: MiniMax Video Factory
site_description: Local video factory + Audiovisual Studio + personal knowledge base, orchestrated over MCP.
docs_dir: docs
theme:
  name: material
  features:
...
nav:
  - Home: index.md
  - Knowledge Base:
      - Design Spec: superpowers/specs/2026-08-07-knowledge-base-mvp-design.md
      - Implementation Plan: superpowers/plans/2026-08-07-knowledge-base-mvp.md
  - Roadmap: ROADMAP.md
...
- **New here?** Start with the [MCP Tools Reference](MCP_TOOLS.md) for the
  full list of available tools, or the
  [Knowledge Base Design Spec](superpowers/specs/2026-08-07-knowledge-base-mvp-design.md)
  for how the knowledge base is architected.
...
Generated directly from docstrings in `src/minimax_mcp/` — do not hand-edit.

## Knowledge base

::: minimax_mcp.knowledge
...
Expected: exits 0 with no warnings (`--strict` turns broken internal links or
missing nav files into errors); `site/index.html` and `site/api/index.html`
exist.
...
Expected: serves on `http://127.0.0.1:8000` — open it and confirm the nav
shows Home / Knowledge Base / Roadmap / MCP Tools Reference / API Reference,
and that `API Reference` renders real function signatures from
`knowledge.py`/`db.py`/etc. (not raw `::: minimax_mcp...` text — that would
mean the `mkdocstrings` plugin isn't picking up the `paths: [src]` config).
Stop the server (Ctrl-C) once confirmed.

- [ ] **Step 7: Commit**

```bash
git add pyproject.toml uv.lock .gitignore mkdocs.yml docs/index.md docs/api.md
git commit -m "docs: add MkDocs + Material + mkdocstrings site (auto-generated docs website)"
```
...
- **Docs website (explicit user priority, added as Task 14):** MkDocs + Material + mkdocstrings turns every markdown doc this plan produces, plus live docstrings from `db.py`/`llm.py`/`rerank.py`/`vault.py`/`knowledge.py`, into one searchable static site (`uv run mkdocs serve`/`build`) — the standard Python-ecosystem answer to "auto-generate a documentation webpage," same stack FastAPI/Pydantic/HTTPX use. Purely additive: no other task depends on it, so it's safe to defer or skip without breaking anything else in this plan.
- **Deferred to Post-MVP** (per spec, not part of this plan): Karakeep→Postgres ingestion, a web UI/site for the knowledge base itself (distinct from the Task 14 docs site, which documents the code, not a product UI), dedup/MOCs beyond the rerank already included, and the "product for other people" phase — see the separate roadmap document.
